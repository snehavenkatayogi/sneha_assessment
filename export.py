...
def parse_data(data: Dict) -> Tuple[Dict, Dict]:
    ...
    visits = { 
        ...
        'visit_start_time': get_iso_time(data['visitStartTime']), 
        ...
        hits.append(
            {
                ...
                'hit_type': hit['type'],
                'hit_timestamp': get_iso_time(int(data['visitStartTime']) + int(hit['time'])),
...
if __name__ == '__main__':
    try:
        ...
        if not sys.stdin.isatty():
            ...
            visits_output_handler = open(default_visits_filename, 'w')
            hits_output_handler = open(default_hits_filename, 'w')
        else:
            # No input given using STDIN, expect input as argument
            parser = argparse.ArgumentParser(description="Process Google Analytics data into separate 'visits.json' and 'hits.json' files")
            parser.add_argument('file', help="Path to GA data file in line-delimited JSON format", type=argparse.FileType('r'))
            parser.add_argument('-ov', help="Visits JSON output file path", type=argparse.FileType('w'), default=default_visits_filename)
            parser.add_argument('-oh', help="Hits JSON output file path", type=argparse.FileType('w'), default=default_hits_filename)
